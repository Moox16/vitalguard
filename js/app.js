...
import {
  ...
  saveReading, getAllReadings, getLatestReadingsAll,
...
let THRESHOLDS = { hr_high:100, hr_low:50, spo2_warn:94, spo2_alert:90, temp_warn:37.5, temp_alert:38.5 };

function loadThresholds() {
  const s = localStorage.getItem('vg_thresholds');
  if (s) THRESHOLDS = { ...THRESHOLDS, ...JSON.parse(s) };
}
function saveThresholds() { localStorage.setItem('vg_thresholds', JSON.stringify(THRESHOLDS)); }
...
export function goTo(name) {
  ...
  if (lb) lb.style.display = meta.live ? 'inline-flex' : 'none';
...
async function renderHome() {
  ...
  if (el('stat-normal'))  el('stat-normal').textContent  = normal;
  if (el('stat-atencao')) el('stat-atencao').textContent = atencao;
  if (el('stat-alerta'))  el('stat-alerta').textContent  = alertCount;
...
function renderAlerts() {
  ...
  if (!alerts.length) {
    if (panel) panel.innerHTML = '<div class="notif-empty">Sem alertas recentes.</div>';
    if (badge) badge.style.display = 'none';
    if (header) header.textContent = 'Notificações';
    if (homeAlerts) homeAlerts.innerHTML = '<div style="color:var(--text-secondary);font-size:13px;padding:8px 0">Sem alertas.</div>';
    return;
  }

  if (badge)  badge.style.display = 'block';
  if (header) header.textContent  = `Notificações (${alerts.length})`;
  if (homeAlerts) homeAlerts.innerHTML = '';
...
function applyRealtimeFilter() {
  document.querySelectorAll('#rt-tbody tr[data-pid]').forEach(row => {
    const matchStatus   = activeFilter === 'todos' || row.dataset.status === activeFilter;
    const matchSearch   = !realtimeSearchQuery || row.dataset.name.includes(realtimeSearchQuery);
    const matchSelected = !selectedPatientId || row.dataset.pid === selectedPatientId;
    row.style.display = matchStatus && matchSearch && matchSelected ? '' : 'none';
  });
}
...
window.handleDeletePatient = async (id, name) => {
  if (!confirm(`Apagar "${name}"? Esta acção remove todos os dados e não pode ser desfeita.`)) return;
  try {
  ...
  } catch (e) { showToast('Erro ao apagar utente.', 'error'); }
};

// ─── Vital cards ──────────────────────────────────────────────
function updateVitalCards(reading) {
  setVital('hr',   reading.heart_rate,  v => v > THRESHOLDS.hr_high || v < THRESHOLDS.hr_low ? 'alert' : 'ok');
...
function setVital(key, value, statusFn) {
  ...
  statusEl.className   = `vital-status vp vp-${s}`;
}
...
async function toggleBluetooth() {
  ...
  if (!selectedPatientId) {
    ...
    if (selector) { selector.style.border = '1.5px solid #E24B4A'; setTimeout(() => selector.style.border = '', 2000); }
    return;
  }
...
async function onBLEData(data) {
  ...
  if (currentReading.heart_rate != null && currentReading.spo2 != null && currentReading.temperature != null) {
    ...
    } catch (e) { console.error('BLE save error:', e); }
  }
}
...
function onBLEDisconnect() {
  ...
  if (lbl) lbl.textContent = 'Ligar dispositivo ESP32';
...
async function renderRegistos() {
  ...
  if (!tbody) return;
  tbody.innerHTML = '<tr class="empty-row"><td colspan="9"><div class="loading"><div class="spinner"></div>A carregar...</div></td></tr>';
  ...
  try {
    ...
    tbody.innerHTML = '';
    if (!patients.length) {
      tbody.innerHTML = '<tr class="empty-row"><td colspan="9">Sem utentes registados.</td></tr>';
      return;
    }
    ...
    const renderRows = (p) => {
      ...
      const notesCell = (editable) => editable ? `
      ...
      if (!pReadings.length) {
        tbody.innerHTML += `<tr>
          <td data-label="Data">—</td><td data-label="Hora">—</td>
          <td>${nameCell()}</td>
          <td data-label="FC">—</td><td data-label="SpO₂">—</td><td data-label="Temp">—</td>
    ...
    console.error(e);
    tbody.innerHTML = '<tr class="empty-row"><td colspan="9">Erro ao carregar registos.</td></tr>';
  }
}
...
window.saveNotes = async (patientId, btn) => {
  ...
  try {
    ...
    renderRegistos();
  } catch (e) { showToast('Erro ao guardar observações.', 'error'); }
};

window.deleteNotes = async (patientId) => {
  ...
  try {
    ...
    renderRegistos();
  } catch (e) { showToast('Erro ao apagar observações.', 'error'); }
};
...
async function exportCSV() {
  try {
    ...
    const rows = [['Data','Hora','Utente','ID Inst.','Data Nasc.','Altura (cm)','Peso (kg)','FC (bpm)','SpO2 (%)','Temp (°C)','Queda','Observações']];
    patients.forEach(p => {
      const pReadings = readings.filter(r => r.patient_id === p.id);
      const base = [p.name, p.institution_id||'', p.dob||'', p.height_cm||'', p.weight_kg||''];
      if (!pReadings.length) { rows.push(['—','—',...base,'—','—','—','—', p.notes||'']); return; }
      pReadings.forEach(r => {
...
function avatarColors(id) {
  const idx = (typeof id === 'string' ? id.charCodeAt(0) : (id||0)) % PALETTE.length;
  return PALETTE[idx];
}
